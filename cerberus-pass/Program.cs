...
var baseAppPath = Path.Join(appData, "CerberusPass");
Directory.CreateDirectory(baseAppPath);
PasswordManager manager = new(baseAppPath);
...
{// "masterpass.txt" exists:
 // prompt user for master-password
    ...
    if (
          ...
          && manager.VerifyMasterPass(userInput)
    )
    {
...
""");
    ...
    switch ((MenuOptions)menuChoice)
    {
        ...
        case MenuOptions.GetOne:
            ...
            var titleToPrint = Console.ReadLine() ?? string.Empty;
            var entry = manager.GetEntry(titleToPrint);
            if (entry is null)
                Console.WriteLine($"Keinen Eintrag mit Title \"{titleToPrint}\" gefunden!");
            else
                Console.WriteLine(entry + $"\t{entry?.Password}");
            break;
        case MenuOptions.Create:
            {
                var masterPassPlain = PromptMasterPass();
                // prompt master-pass check
                if (!manager.VerifyMasterPass(masterPassPlain))
                {
...
static string PromptMasterPass()
{
